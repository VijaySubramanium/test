...
@Component({
  selector: 'app-attendance-trainer',
  templateUrl: './attendance-trainer.component.html',
  styleUrls: ['./attendance-trainer.component.css'],
})
export class AttendanceTrainerComponent implements OnInit {
  user: any;
  id_User: any;
  roleId_User: any;
  id_Project: any;
  ...
  openPopup(date: any, userId: any) {
    var now = moment();
    let ind = this.attendenceData.findIndex(
      (x) => x.studentId === userId
    );
    let index = this.attendenceData[ind].attendance.findIndex(
  ...
  getUserProjectLists() {
    let params = {
      userid: this.id_User,
      roleid: this.roleId_User,
    };
    $('.spinner').show();
    this.attendanceService.getUserProjectList(params).subscribe((response) => {
      $('.spinner').hide();
  ...
  getUserProgramLists(project: any) {
    $('#programList').val('');
    ...
    $('.spinner').show();
    this.attendanceService.getUserProgramList(params).subscribe((response) => {
      $('.spinner').hide();
  ...
  /* Display Course List*/
  getUserCourseLists(program: any) {
    $('#courseList').val('');
    this.courseDetails = null;
    $('#batchList').val('');
    this.batchDetails = null;
    ...
    $('.spinner').show();
    this.attendanceService.getUserCourseList(params).subscribe((response) => {
      $('.spinner').hide();
  ...
  /* Display Batch List*/
  getUserBatchLists(course: any) {
    $('#batchList').val('');
    this.batchDetails = null;
    let courseDetails = course.split('_', 2);
    ...
    $('.spinner').show();
    this.attendanceService.getUserBatchList(params).subscribe((response) => {
      $('.spinner').hide();
